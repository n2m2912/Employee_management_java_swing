...
public class Job extends javax.swing.JFrame {
    ...
    public Job(int id,String btName) {
        this.id = id;
        this.btName = btName;
        initComponents();
        setInfoDialog();
    ...
    public void loadJob(int id){
        Jobs emp = jobBLL.findJobsBll(id);
        txtName.setText(emp.getJobName());
    }
    ...
    private void btSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btSaveActionPerformed
        ...
        if(btName == "New" && !error){
            Jobs model = new Jobs(name);
            jobBLL.addNewJobsDAL(model);
            editTableData(model,btName);
        }
        else if(btName == "Edit" && !error){
            id = Integer.valueOf(JobList.tb.getModel().getValueAt(JobList.tb.getSelectedRow(), 0).toString());
            Jobs model = new Jobs(id,name);
            jobBLL.editJobsDAL(model);
            editTableData(model,btName);
        }
    ...
    public void editTableData(Jobs derp,String btName){
        int row = 0; 
        DefaultTableModel model = (DefaultTableModel) JobList.tb.getModel();
        if (btName == "Edit"){
            row = JobList.tb.getSelectedRow();
        }
        if (btName == "New"){
            row = JobList.tb.getRowCount();
            model.addRow(new Object[0]);
        }
        model.setValueAt(derp.getId(), row, 0);
        model.setValueAt(derp.getJobName(), row, 1);
        JobList.btE.setEnabled(false);
    }
    /**
